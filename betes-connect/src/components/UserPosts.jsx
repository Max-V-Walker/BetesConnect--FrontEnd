    ...
    return (
      <div className='profilePosts'>
        <div key={post._id} className='userPosts'>
          <img src={post.author.profilePhoto} alt={user.username} style={{height: '50px', width: '50px'}} className='userPic'/>
          <h5>{post.headline}</h5>
          ...
          <i className="btn" onClick={() => likePost(post, user.username)}>{heart} <span className='badge badge-light'>{post.likes.length}</span></i>
